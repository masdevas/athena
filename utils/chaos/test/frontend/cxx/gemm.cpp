  ...
  for (int i = 0; i < M; i++) {
    ...
    for (int k = 0; k < K; k++) {
      ...
      for (int j = 0; j < N; j++) {
        ...
        // CHECK-NEXT: %17 = "std.load"(%arg2, %16) : (memref<?xf32>, index) ->
        // f32 CHECK-NEXT: %18 = "std.constant"() {value = 1.000000e+00 : f32} :
        // () -> f32 CHECK-NEXT: %19 = "std.index_cast"(%arg6) : (index) -> i32
        ...
        // CHECK-NEXT: %24 = "std.load"(%arg0, %23) : (memref<?xf32>, index) ->
        // f32 CHECK-NEXT: %25 = "std.mulf"(%18, %24) : (f32, f32) -> f32
        ...
        // CHECK-NEXT: %31 = "std.load"(%arg1, %30) : (memref<?xf32>, index) ->
        // f32 CHECK-NEXT: %32 = "std.mulf"(%25, %31) : (f32, f32) -> f32
        ...
        // CHECK-NEXT: "std.store"(%33, %arg2, %38) : (f32, memref<?xf32>,
        // index) -> ()
...
int main() {
  ...
  // CHECK-NEXT: %4 = "std.alloc"(%3) {alignment = 4 : i64} : (index) ->
  // memref<?xf32>
  ...
  // CHECK-NEXT: %9 = "std.alloc"(%8) {alignment = 4 : i64} : (index) ->
  // memref<?xf32>
  ...
  // CHECK-NEXT: %14 = "std.alloc"(%13) {alignment = 4 : i64} : (index) ->
  // memref<?xf32>
  ...
  // CHECK-NEXT: "std.call"(%4, %9, %14, %15, %16, %17) {callee =
  // @_Z5sgemmPKfS0_Pfiii} : (memref<?xf32>, memref<?xf32>, memref<?xf32>, i32,
  // i32, i32) -> ()
