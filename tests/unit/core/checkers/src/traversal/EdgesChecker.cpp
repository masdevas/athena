...
std::ostream& operator<<(std::ostream& stream, const Edge& edge) {
  stream << edge.startNode << ' ' << edge.endNode << ' ' << edge.mark << std::endl;
  return stream;
}
  ...
  for (auto& cluster : traversal.getClusters()) {
    for (auto& nodeState : cluster.content) {
      for (auto& outputDependence : nodeState.output) {
        auto edge = Edge{nodeState.nodeIndex, outputDependence.nodeIndex, outputDependence.mark};
        edges.emplace(edge);
      }
      for (auto& inputDependence : nodeState.input) {
        auto edge = Edge{inputDependence.nodeIndex, nodeState.nodeIndex, inputDependence.mark};
        edges.emplace(edge);
      }
    }
  }
...
bool checkEdges(const core::Traversal& traversal,
                const std::set<Edge>& edges) {
  auto traversalEdges = createEdgesFromTraversal(traversal);
  if (edges.size() != traversalEdges.size()) {
    std::cerr << "Edges count doesn't equal to traversal edges count" << std::endl;
    std::cerr << "Edges count: " << edges.size() << std::endl;
    std::cerr << "Traversal edges count: " << traversalEdges.size() << std::endl;
