...
//#include <athena/core/OutputNodeInternal.h>
...
//  auto* tensor =
//      new proto_graph::Tensor(serializeTensor(inner::getTensorFromNode(node)));
...
//  }
//  auto& outputNodes = std::get<std::vector<OutputNodeInternal>>(owningStorage);
...
//      serializeNode(savedGraph.add_nodes(), *node_dyncast<Node*>(syncNode));
//    } else if (syncNode->getType() == NodeType::INPUT) {
//      serializeInputNode(savedGraph.add_input_nodes(),
//                         *node_dyncast<InputNodeImpl*>(syncNode));
...
//                          *node_dyncast<OutputNodeInternal*>(syncNode));
//    } else {
//      new FatalError(
//          ATH_NOT_IMPLEMENTED,
//          "Attempt to serialize unsupported node type: ", syncNode->getName());
//    }
//  }
//
...
//    InputNodeImpl n(getShape(node.tensor()), protoToNativeDataType(node.tensor()),
...
//    OutputNodeInternal n(DataType::UNDEFINED, ctx, node.name());
//    graph.addNode(n);
//    nodesRemap[node.index()] = n.getNodeIndex();
//  }
//
