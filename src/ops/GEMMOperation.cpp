...
inner::Tensor*
GEMMOperation::getResultTensor(core::Context& context,
...
core::inner::Tensor*
GEMMOperation::getDerivativeTensor(core::Context& context,
...
void GEMMOperation::gen(
  ...
  if (operationArguments[0]->getDataType() == DataType::FLOAT) {
    auto* options = new GEMMOptions<float>{mTransposeA, mTransposeB, 1.0f, 0.f};
    opts = static_cast<void*>(options);
  } else if (operationArguments[0]->getDataType() == DataType::DOUBLE) {
    auto* options = new GEMMOptions<double>{mTransposeA, mTransposeB, 1.0, 0.};
    opts = static_cast<void*>(options);
  } else {
...
void GEMMOperation::genDerivative(
    int order, core::AbstractGenerator& g, core::inner::Tensor& operationResult,
  ...
  if (operationArguments[0]->getDataType() == DataType::FLOAT) {
    ...
    if (mTransposeB)
      options->transposeB = !options->transposeB;
    if (mTransposeA)
      options->transposeA = !options->transposeA;
    opts = static_cast<void*>(options);
  } else if (operationArguments[0]->getDataType() == DataType::DOUBLE) {
