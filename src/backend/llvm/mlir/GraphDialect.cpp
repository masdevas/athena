...
void athena::backend::llvm::AllocaOp::build(
    ...
    const athena::core::inner::Tensor &tensor,
    int64_t node_id,
    StringRef node_name,
    int64_t cluster_id) {
    std::vector<int64_t> shape;
    std::transform(tensor.getShapeView().begin(), tensor.getShapeView().end(),
                   std::back_inserter(shape), [&](const size_t item) {
    ...
    auto tensorAddrAttr =
        builder->getI64IntegerAttr(tensor.getVirtualAddress());
...
void athena::backend::llvm::AddOp::build(mlir::Builder *builder,
                                         OperationState &result,
                                         const mlir::Value &a,
                                         const mlir::Value &b,
...
void athena::backend::llvm::MulOp::build(mlir::Builder *builder,
                                         OperationState &result,
                                         const mlir::Value &a,
                                         const mlir::Value &b,
                                         const athena::core::inner::Tensor &c,
