...
void MLIRGenerator::generateImpl(std::string& name, Tensor& a, Tensor& b,
  ...
  if (name == "add") {
    ...
    mlir::Value cVal = mBuilder.create<AddOp>(mBuilder.getUnknownLoc(), aVal,
                                              bVal, c, 1, "temp_node_name", 0);
    mTensorValueMap[c.getVirtualAddress()] = cVal;
  }
}
void MLIRGenerator::generateImpl(std::string& string, Tensor& a,
                                 uint64_t scaleA, Tensor& b, uint64_t scaleB,
                                 Tensor& c) {}
void MLIRGenerator::generateImpl(std::string& string, void* pVoid, Tensor& a,
                                 Tensor& b, Tensor& c) {}

MLIRGenerator::MLIRGenerator() : mBuilder(&mContext) {
  mModule = mlir::ModuleOp::create(mBuilder.getUnknownLoc());
  mlir::FuncOp function =
      mlir::FuncOp::create(mBuilder.getUnknownLoc(), "evaluate",
