...
AbstractNode::AbstractNode(const AbstractNode& rhs)
    : mTensor(rhs.mTensor), mContext(rhs.mContext), mName(rhs.mName),
...
AbstractNode::AbstractNode(AbstractNode&& rhs) noexcept
    : mTensor(rhs.mTensor), mContext(rhs.mContext), mName(std::move(rhs.mName)),
      mGraphIndex(rhs.mGraphIndex), mNodeIndex(rhs.mNodeIndex),
      mInputsCount(rhs.mInputsCount) {
  inner::getNodeTable(*mContext)[mNodeIndex] = this;
  rhs.fullClear();
}
AbstractNode::AbstractNode(TensorShape shape, DataType dataType,
...
ShapeView AbstractNode::getShapeView() const { return mTensor->getShapeView(); }
...
DataType AbstractNode::getDataType() const { return mTensor->getDataType(); }
size_t AbstractNode::getNodeIndex() const { return mNodeIndex; }
size_t AbstractNode::getGraphIndex() const { return mGraphIndex; }
size_t AbstractNode::getInputsCount() const { return mInputsCount; }
std::string_view AbstractNode::getName() const { return mName; }
std::string& AbstractNode::name() { return mName; }
const std::string& AbstractNode::name() const { return mName; }
...
AbstractNode::AbstractNode(Context& context, std::string name)
    : mTensor(inner::getNullTensor(context)), mContext(&context),
      mName(std::move(name)), mGraphIndex(inner::kKUndefinedIndex),
      mNodeIndex(inner::getNodeTable(*mContext).registerRecord(this)),
      mInputsCount(0) {}
