...
AbstractNodeInternal::AbstractNodeInternal(utils::WeakPtr<ContextInternal> context, utils::Index publicNodeIndex,
    utils::String name) : Entity(std::move(context), publicNodeIndex, std::move(name)), mTensorIndex{} {}

AbstractNodeInternal::AbstractNodeInternal(utils::WeakPtr<ContextInternal> context,
    utils::Index publicNodeIndex, utils::Index tensorIndex,
    utils::String name) : Entity(std::move(context), publicNodeIndex, std::move(name)), mTensorIndex(tensorIndex) {}

AbstractNodeInternal::~AbstractNodeInternal() {
  //TODO
}

void AbstractNodeInternal::after(const AbstractNodeInternal& node, EdgeMark mark) const {}

void AbstractNodeInternal::before(const AbstractNodeInternal& node, EdgeMark mark) const {}
