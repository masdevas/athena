...
void initQueue(std::queue<size_t>& queue, const OwningStorage& owningStorage,
               const SyncStorage& syncStorage, Context& context) {
...
Graph::Graph(Graph&& rhs) noexcept
    : mSyncStorage(std::move(rhs.mSyncStorage)),
      ...
      mTopology(std::move(rhs.mTopology)), mContext(rhs.mContext),
      mGraphIndex(rhs.mGraphIndex), mTraversal(std::move(rhs.mTraversal)) {
...
template <typename TemplateNodeType>
void Graph::saveRealNode(TemplateNodeType& node, bool isRepairedNode,
                         bool isErase) {
  if (isErase) {
...
const SyncStorage& Graph::getSyncStorage() const { return mSyncStorage; }
const OwningStorage& Graph::getOwningStorage() const { return mOwningStorage; }
const Topology& Graph::getTopology() const { return mTopology; }
...
void Graph::link(const AbstractNode& startNode, const AbstractNode& endNode,
                 EdgeMark mark) {
...
size_t Graph::countSyncNodes() const { return mSyncStorage.size(); }
size_t Graph::getGraphIndex() const { return mGraphIndex; }
bool Graph::isValidTraversal() const { return mTraversal.isValidTraversal(); }
    ...
    while (!currentQueue.empty()) {
      ...
      while (edgeIterator != mTopology.end() &&
        ...
        if (inputCount == targetInputCount) {
          ...
          FatalError(ATH_FATAL_OTHER, "traverse() in Graph: ", mGraphIndex,
                     ". Graph has cycle(s)");
        }
        visits[edgeIterator->endNodeIndex].input.emplace(nodeIndex,
                                                         edgeIterator->mark);
      ...
      default:
        FatalError(ATH_NOT_IMPLEMENTED, "Undefined NodeType in traverse()");
      }
      ++cluster.nodeCount;
    }
  ...
  for (auto& cluster : mTraversal.getClusters()) {
    ...
    for (auto& nodeDep : actionNodes) {
      ...
      inner::setResultTensor(
          node, std::shared_ptr<inner::Tensor>(
    ...
    for (auto& nodeDep : outputNodes) {
      auto& node = node_cast<OutputNode&>(
          *inner::getNodeTable(*mContext)[nodeDep.nodeIndex]);
      auto& parentNode =
          *inner::getNodeTable(*mContext)[nodeDep.input.begin()->nodeIndex];
      inner::setResultTensor(node, inner::getTensorPtrFromNode(parentNode));
    }
  }
}
} // namespace athena::core
...
Context& getContext(athena::core::Graph& graph) { return *(graph.mContext); }
} // namespace athena::core::inner
