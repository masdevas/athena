...
Tensor::Tensor(Context& context, size_t id, size_t recordIndex,
...
const TensorShape& Tensor::getShape() const {
  if (mShapeOffset != 0) {
    FatalError(ATH_NOT_IMPLEMENTED, "getShape is not supported for subtensors");
  }
...
size_t Tensor::getSize() const {
  return getAllocationTable(*mContext)[mRecordIndex].getSize() /
         mShapePartialProduct;
}
