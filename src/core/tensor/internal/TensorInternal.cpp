...
TensorInternal::TensorInternal(utils::WeakPtr<ContextInternal> context, utils::Index publicIndex, DataType dataType, TensorShape shape)
  : Entity(std::move(context), publicIndex), mDataType(dataType), mShape(std::move(shape)), mVirtualAddress(mContext.lock()->registerTensor(*this)) {}

//TensorInternal::TensorInternal(DataType dataType, TensorShape shape, Context& context)
//    : mContext(&context),
...
//TensorInternal::TensorInternal(Context& context, size_t id, size_t recordIndex,
...
DataType TensorInternal::getDataType() const {
  return mDataType;
}

ShapeView TensorInternal::getShapeView() const {
  return ShapeView(mShape);
}

ShapeView TensorInternal::getSubShapeView(size_t offset) const {
#ifdef DEBUG
  if (mShape.begin() + offset > mShape.end()) {
    new utils::FatalError(utils::ATH_BAD_ACCESS, "Bad SubShapeView constructing");
  }
#endif
  return ShapeView(mShape.begin() + offset, mShape.end());
}

const TensorShape& TensorInternal::getShape() const {
  return mShape;
}

size_t TensorInternal::getSize() const {
