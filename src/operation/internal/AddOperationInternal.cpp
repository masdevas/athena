...
utils::Index AddOperationInternal::createResultTensor(utils::SharedPtr<core::internal::ContextInternal> context,
...
void AddOperationInternal::gen(utils::SharedPtr<core::internal::ContextInternal> context, core::Generator& generator,
                 std::vector<utils::Index>& operationArguments) const {
  //TODO call generator
}

std::tuple<utils::Index, std::vector<core::internal::Edge>, std::vector<utils::Index>> AddOperationInternal::genDerivative(const core::NodeState* currentNodeState,
  ...
  auto constantLoaderIndex = context->create<loaders::internal::ConstantLoaderInternal>(context, context->getNextPublicIndex(), 1.0, (std::string("AddOp_IdentityLoader") + std::to_string(context->getNextPublicIndex())).data());
  auto nodeWithIdentityTensor = context->create<core::internal::InputNodeInternal>(context, context->getNextPublicIndex(),
                                                                                   argumentNode.getTensorPtr()->getShape(), argumentNode.getTensorPtr()->getDataType(),
                                                                                   true, constantLoaderIndex, (std::string("AddOp_IdentityNode") + std::to_string(context->getNextPublicIndex())).data());
  auto mulOperationIndex = context->create<MulOperationInternal>(context, context->getNextPublicIndex(), (std::string("AddOp_MulOperation") + std::to_string(context->getNextPublicIndex())).data());
  auto mulNodeIndex = context->create<core::internal::NodeInternal>(context, context->getNextPublicIndex(), mulOperationIndex, (std::string("AddOp_MulNode") + std::to_string(context->getNextPublicIndex())).data());
...
size_t AddOperationInternal::getOperandsCount() const {
  return 2;
}
}
