...
template <typename... Args>
String mergeToString(Args&&... messages) {
  std::stringstream ss;
...
ATH_FORCE_INLINE Error::Error(int32_t errorCode, Args&&... messages)
    : mErrorCode(errorCode), mErrorMessage(mergeToString(std::forward<Args>(messages)...)) {}

ATH_UTILS_EXPORT ATH_FORCE_INLINE std::ostream& operator<<(std::ostream& stream, const Error& err) {
