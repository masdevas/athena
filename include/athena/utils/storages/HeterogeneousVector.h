...
public:
  ...
  explicit HeterogeneousVector(size_t capacity, size_t elementAverageSize, Allocator allocator = Allocator());
...
template <typename BaseClass>
HeterogeneousVector<BaseClass>::HeterogeneousVector(HeterogeneousVector<BaseClass>&& rhs) : mAllocator(rhs.mAllocator), mElementAverageSize(rhs.mElementAverageSize), mNextIndex(rhs.mNextIndex),
  mCapacity(rhs.mCapacity), mOffsets(rhs.mOffsets), mBytes(rhs.mBytes) {
...
utils::Index HeterogeneousVector<BaseClass>::emplaceBack(Args&&... args) {
  ...
  new (static_cast<unsigned char*>(mBytes) + mOffsets[resIndex]) Type(std::forward<Args>(args)...);
  return resIndex;
}

template <typename BaseClass>
HeterogeneousVector<BaseClass>::HeterogeneousVector(size_t capacity, size_t elementAverageSize, Allocator allocator) : mAllocator(std::move(allocator)),
  mElementAverageSize(elementAverageSize),
  mCapacity(0), mNextIndex(0),
  mOffsets(nullptr),
  mBytes(nullptr) {
  resize(capacity);
}
...
void HeterogeneousVector<BaseClass>::destroy(bool isDestruct) {
  if (isDestruct) {
    for (size_t index = 0; index < mNextIndex; ++index) {
      reinterpret_cast<BaseClass*>(static_cast<unsigned char*>(mBytes) + mOffsets[index])->~BaseClass();
    }
  }
...
template <typename BaseClass>
size_t HeterogeneousVector<BaseClass>::calculateRequiredCapacity(size_t addedElementSize) {
...
void HeterogeneousVector<BaseClass>::resize(size_t capacity) {
  ...
  auto offsets = static_cast<OffsetType*>(mAllocator.allocateBytes((capacity + 1) * sizeof(OffsetType)));
  auto bytes = static_cast<byte*>(mAllocator.allocateBytes(capacity * mElementAverageSize));
  ...
  std::memset(offsets + mCapacity, 0, (capacity + 1 - mCapacity) * sizeof(OffsetType));
...
template <typename BaseClass>
size_t HeterogeneousVector<BaseClass>::size() const { return mNextIndex; }

template <typename BaseClass>
template <typename Type>
Type& HeterogeneousVector<BaseClass>::get(size_t index) {
  return *reinterpret_cast<Type*>(static_cast<unsigned char*>(mBytes) + mOffsets[index]);
}
...
bool HeterogeneousVector<BaseClass>::isAbleToContain() const {
  auto tmp = mCapacity > mNextIndex && mCapacity * mElementAverageSize >= mOffsets[mNextIndex] + sizeof(Type);
  return tmp;
}
