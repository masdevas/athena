...
public:
  OperationInternal(utils::WeakPtr<ContextInternal> context, utils::Index publicNodeIndex, utils::String name = utils::String(""));

  ~OperationInternal() override = default;

  [[nodiscard]] virtual utils::Index createResultTensor(utils::SharedPtr<ContextInternal> context,
                  std::vector<TensorInternal*> tensorIndexes) const = 0;

  virtual void gen(utils::SharedPtr<ContextInternal> context, core::Generator& generator,
                   std::vector<utils::Index>& operationArguments) const = 0;

  virtual std::tuple<utils::Index, std::vector<core::internal::Edge>, std::vector<utils::Index>> genDerivative(
      const core::NodeState* currentNodeState,
      size_t indexOfOutputDependence, utils::Index gradientGraphFinalNodeIndex) const = 0;
