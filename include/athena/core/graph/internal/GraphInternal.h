...
public:
  explicit GraphInternal(utils::WeakPtr<ContextInternal> context, utils::Index publicGraphIndex,
  ...
  utils::Index create(Args&&... args) {
    auto index = mContext.lock()->create<TemplateNodeTypeInternal>(mContext.lock(), mContext.lock()->getNextPublicIndex(), std::forward<Args>(args)...);
    // TODO try to use "enable shared from this" for deleting  "mContext, mContext.lock()->getNextPublicIndex()"
...
private:
  ...
  void bypassDependenceOfCurrentNodeState(const NodeState& currentNodeState, size_t currentClusterIndex, size_t currentNodeStateIndex,
                                          std::unordered_map<utils::Index, NodeState>& nodeStates,
                                          std::unordered_map<utils::Index, NodeStateIndex>& traversedNodeStates);

  void initInputNodeStates(std::unordered_map<utils::Index, NodeState>& isPartOfWayToUnfrozenFlags) const;

  std::tuple<utils::Index, std::unordered_map<utils::Index, utils::Index>> createGradientGraph() const;

  utils::Index createInitialGradientNode(const NodeState* nodeStatePtr) const;

  utils::Index accumulateOutputNodes(GraphInternal& gradient, const NodeState* nodeStatePtr, const std::unordered_map<const NodeState*, utils::Index>&
          mapNodeStateToFinalGradientIndex) const;

  void mergeEdges(const std::vector<core::internal::Edge>& edges);

  utils::Index createWeightChangingGraph(const std::unordered_map<utils::Index, utils::Index>& mapInputNodes);
