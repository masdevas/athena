...
public:
   ...
   */
  explicit Graph(utils::SharedPtr<internal::ContextInternal> contextInternal, utils::Index publicGraphIndex);
  ...
  utils::Index create(Args&&... args) {
    return mContext->getRef<internal::GraphInternal>(mPublicIndex).create<typename TemplateNodeType::InternalType>(
...
template <>
struct ATH_CORE_EXPORT Wrapper<Graph> {
  using PublicType = Graph;
};

template <>
struct Returner<Graph> {
